...
$média = \sum_{i=1}^n{x_i}/n$ para observações $\{x_1, x_2, x_3, ..., x_n\}$
...
# Algebra com Esperan~, Variância e Covariância
...
espaco_amostral <- espaco_amostral %>%
  rename("resultado do primeiro dado" = Var1) %>%
  rename("resultado do segundo dado" = Var2)
espaco_amostral$X <- espaco_amostral$`resultado do primeiro dado` + espaco_amostral$`resultado do segundo dado`
espaco_amostral$Y <- pmax(espaco_amostral$`resultado do primeiro dado`, espaco_amostral$`resultado do segundo dado`)

# Criar a tabela
tabela <- kable(espaco_amostral, caption = "Tabela 2.5: Tabela representando a soma (X) and o maior valor (Y) do lançamento de dois dados de quatro lados") %>%
